...
def main():
    ...
    root = ET.Element('div', attrib={'xmlns': 'http://www.w3.org/1999/xhtml', 'class': 'container'})
    ...
    for key in imgs.keys():
        ...
        # add a new icon
        icon = ET.SubElement(div, 'svg', attrib={'width': '100%', 'height': '100%', 'xmlns': 'http://www.w3.org/2000/svg'})  # width=100 height=60
        ...
        icon_text = ET.SubElement(icon, 'text', attrib={'x': '50', 'y': '65', 'class': 'label'})
        icon_text.text = key
    
    # write the SVG to a file
    tree.write(os.path.join(DIR_PATH, OUTPUT_FILE))

    # delete the downloaded icons
    for key in imgs.keys():
